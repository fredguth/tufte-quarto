...
=======
I.  **Knowledge is a set of beliefs, quantifiable by real numbers and dependent on prior evidence [@sowinski2016; @caticha2008; @jaynes2003]:** Let $S_i \in \Sigma_S$ be sentences about the world.  Given any two statements $S_1$, $S_2$, the agent must be able to say that $S_1$ is more plausible than $S_2$, or that $S_2$ is more plausible than $S_1$ or that $S_1$ and $S_2$ are equally plausible. Thus we can list statements in an increasing plausibility order. Real numbers can represent this transitive ordering.[^37][]{#beliefs label="beliefs"}
>>>>>>> e27700f (prob with MathJax problems)
